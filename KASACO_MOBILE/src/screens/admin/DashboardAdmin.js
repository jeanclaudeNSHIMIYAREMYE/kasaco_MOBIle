...
export default function DashboardAdmin() {
  ...
  const fetchStats = async () => {
    try {
      setLoading(true);
      const [utilisateurs, voitures, reservations, marques, modeles] = await Promise.all([
        UtilisateurService.getAllUtilisateurs().catch(() => []),
        VoitureService.getAllVoitures().catch(() => []),
        ReservationService.getAllReservations().catch(() => []),
        MarqueService.getAllMarques().catch(() => []),
        ModeleService.getAllModeles().catch(() => [])
      ]);
  ...
  // STAT CARDS - Utilisation d'icônes valides
  const statCards = [
  ...
  // QUICK ACTIONS - Correction des icônes invalides
  const quickActions = [
